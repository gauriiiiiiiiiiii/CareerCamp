...
# **Project Tech Stack & Rationale**

## **1. Frontend Framework**
**Tech:** React + Vite
**Why:**
* React offers **component-based architecture** → maintainability & reusability.
* Vite provides **fast dev server startup** and **HMR** (Hot Module Replacement) → quicker iteration vs CRA.
* **Tree-shaking by default** → smaller production bundle size.
...
**Tech:** TailwindCSS
**Why:**
* **Utility-first styling** → minimal context switching between CSS & JS.
* Consistent design system via **pre-defined spacing, colors, breakpoints**.
* **PurgeCSS** removes unused classes → smaller final CSS footprint.

---

## **3. State Management**
**Tech:** Redux Toolkit (RTK)
**Why:**
* Eliminates **boilerplate** compared to classic Redux.
* Built-in **Immer** for immutable state updates.
* Scales well for large projects → avoids **prop drilling**.
...
* Smart **invalidation** → avoids unnecessary calls.
* Built-in **loading & error states** → less manual UI logic.

---

## **5. API Key Handling**

**Current:** `.env` in dev, direct call in frontend (plan to move to backend proxy)
**Why:**
* **Fast prototyping**, minimal setup.
* Future shift to **serverless/API proxy** → secure key, control rate limits.
...
* Error handling via **error** & **isFetching** from RTK Query.
...
**Why choices matter:**
* Vite’s **Rollup-based pipeline** → aggressive code-splitting.
* **Dynamic imports** for heavy components → faster initial load.
* **Preloading critical assets** → reduced TTFB (Time to First Byte).

---

## **8. Scalability Plan**
**Why your tech enables growth:**
* **Modular components** → easier to add features without breaking existing ones.
* RTK Query scales with **multiple endpoints** & microservices.
* Tailwind’s **consistent design** speeds up onboarding for new devs.
